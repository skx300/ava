...
* 
* Alain de Cheveign? CNRS/Ircam	Dec 2001
* (c) 2001 CNRS
 * Return the flag for each period estiamte whether the each one is deeper than the threshold or not.
 * Changed by Luwei 29/06/2016
*/
...
//LUWEI: add the flag for indicating whether the period is under the threshold.
//1: deeper than the treshold; 0: not deeper than the threshold
#define GOOD_FLG plhs[1]    
...
static void ddftoperiods(
				...
				double *bp,			/* bounds */
				double *prdp, 		/* periods */
                double *good_flg,       /*LUWEI: good flag */ 
			...
			) 
{
    ...
    double flg = 0; //LUWEI: assume the flag is 0
	...
	for (k=0;k<n;k++) {											/* for each col */
        ...
        /*----LUWEI ADDED START-----*/
        if (min < thresh){
            flg = 1; //LUWEI: find the one deeper than the threshold
        }
        /*----LUWEI ADDED END-----*/
